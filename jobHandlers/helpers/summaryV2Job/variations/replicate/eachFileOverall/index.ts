...
export async function replicateSummarizeEachFileOverall(
  ...
  try {
    p("REPLICATE Summarize Each File Overall"); // for console debugging...
    ...
    for (let fIndex = 0; fIndex < filesToText.length; fIndex++) {
      ...
      if (format === "bullet-points") {
        ...
        // ***
        console.log("DEBUG LLaMa 2 completion", completion);
        const finalBulletPointsCompletionText: string =
          // @ts-ignore
          completion?.join("") || "ERROR: No Content";
